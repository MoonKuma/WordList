...
def record_result(word, result, current_dict, total_word_dict, pass_rate):
    ...
    pass_rate1 = float(current_dict[word]['correct_times'])/current_dict[word]['test_times']
    pass_rate2 = float(total_word_dict[word]['correct_times'])/total_word_dict[word]['test_times']
    if current_dict[word]['test_times'] > 10 and pass_rate1 < pass_rate and current_dict[word]['status'] == 1:
        current_dict[word]['status'] = 0
    if total_word_dict[word]['test_times'] > 20 and pass_rate2 < pass_rate and total_word_dict[word]['status'] == 1:
        total_word_dict[word]['status'] = 0
    return 1
